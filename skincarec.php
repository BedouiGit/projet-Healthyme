...
public function listProbSol($ID){
    $sql="SELECT p.ID, p.problem, s.solution FROM problems p JOIN solutions s ON p.ID=s.ID  WHERE p.ID = :id ";
    echo $sql;
    $db = config::getConnexion();
    try{
        $req = $db->prepare($sql);
...
class productc{
    ...
    public function listproducts($skintype){
        $sql="SELECT * from products where skintype like'%$skintype%";
        $pdo= config::getConnexion();
        try{
        $list=$pdo->query($sql);
        return $list;
